...
// TensorFlow.js import made optional to prevent Alpine Linux issues
let tf = null;
try {
  tf = require('@tensorflow/tfjs-node');
} catch (error) {
  console.warn('⚠️ TensorFlow.js not available, using simulation mode:', error.message);
}
...
class MLRepairCostEstimationService {
  ...
  estimateRepairTime(damageAnalysis) {
    const baseHours = (damageAnalysis.totalDamageFound || 0) * 2; // 2 hours per damage point
